...
#
# ─── Wozu ──────────────────────────────────────────────────────────────────────
# Die CI lässt bei einer reinen Doku-Änderung die TEUREN Schritte weg (Installation,
# Browser-Test, Abbild-Bau) und fährt nur die Hygiene. Der Gewinn ist gemessen:
# ein `tests`-Bein kostet 55–62 s, davon 11 s Installation, 5 s Chrome und 36–41 s
# die Suite (zweimal, für die Wiederholbarkeit). Übrig bleiben ~7 s.
#
...
#       weiter: Versionsgleichstand pyproject/CHANGELOG, „i18n/README.de.md folgt
#       der Struktur von README.md", „backlog/README.md ist aktuell",
#       Pflichtdateien, Namens- und Adress-Sperrliste, Fremdressourcen.
#       Genau deshalb darf CHANGELOG.md mit.
#
#   docs/**, i18n/**, backlog/**
...
#
#   pyproject.toml          Version, Abhängigkeiten, Extras, requires-python —
#                           Verhalten, nicht Prosa.
...
#   app/**, tests/**, scripts/**, alles andere   Code.
#
...
if [[ "${1:-}" == "--seit" ]]; then
    ...
    # ZIEL. Ein `app/x.py` -> `docs/x.py` sähe dann wie eine reine Doku-Änderung
